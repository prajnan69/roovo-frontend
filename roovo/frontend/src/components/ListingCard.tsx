...
const ListingCard: React.FC<ListingCardProps> = ({ listing }) => {
  ...
  const images = listing.images.map((src, index) => ({ id: index, img: src }));

  return (
    // Card container: Smaller width, flex-shrink-0 to prevent squishing
    <div className="cursor-pointer w-48 flex-shrink-0 isolate">
      {/* --- Image Carousel --- */}
      <div className="relative rounded-2xl aspect-square group">
        <Stack cardsData={images} cardDimensions={{ width: 192, height: 192 }} />
        ...
        {isGuestFavourite && (
          ...
          >
            <div className="relative flex items-center">
              <IconStarFilled className="h-3 w-3 text-white drop-shadow-lg" />
              <AnimatePresence>
                {isBadgeHovered && (
                  <motion.div
                    ...
                    className="absolute left-full ml-4 px-3 py-1 rounded-full bg-white text-slate-900 text-xs font-bold shadow-lg whitespace-nowrap"
                  >
                    Guest favourite
                  </motion.div>
                )}
...
export const SkeletonCard: React.FC = () => (
  <div className="animate-pulse w-56 flex-shrink-0">
